...
class AlgoStrategy(gamelib.AlgoCore):
    def __init__(self):
        ...
        self.VERTICAL = 2
        self.MAPSIZE = 27
        self.spawnLocation = [[1, 12], [5, 8], [26, 12], [22, 8]]
    ...
    def choose_intercepter(
        ...
        a = []
        for [x, y] in self.spawnLocation:
            a.append(
                [x, y, self.simulate_intercepter_score(game_map, game_state, x, y)]
            )
    ...
    def simulate_intercepter_score(
        self, game_map: gamelib.GameMap, game_state: gamelib.GameState, x: int, y: int
    ):
        score = 0
        ...
        while y < 16:
            ...
            nx, ny = self.next_step(game_map, x, y, prevMovDir, targetEdge)
            if nx == x:
                prevMovDir = self.VERTICAL
            else:
                prevMovDir = self.HORIZONTAL

            score += self.heat_map[nx][ny]
            x = nx
            y = ny
        return score
    ...
    def choose_direction(
        ...
        for i, j in step:
            i += x
            j += y
            if not game_map.in_arena_bounds([i, j]) or (
                game_map[i, j] is not None and len(game_map[i, j]) != 0
            ):
                continue
